...
# Load data
def load_data():
    global sp_data, role_permissions, teams, team_invitations, player_teams
    try:
...
class TournamentView(discord.ui.View):
    ...
    async def start_tournament(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            # Auto-fill with bots to make even bracket
            if tournament.mode == "2v2":
                current_teams = len(tournament.players) // 2
                # Find next power of 2
                target_teams = 1
                while target_teams < current_teams:
                    target_teams *= 2

                teams_to_add = target_teams - current_teams
                for i in range(teams_to_add):
                    # Create bot team
                ...
                # Find next power of 2
                target_players = 1
                while target_players < len(tournament.players):
                    target_players *= 2

                players_to_add = target_players - len(tournament.players)
            ...
            if tournament.mode == "2v2":
                for i, match in enumerate(tournament.rounds[0], 1):
                    team_a, team_b = match
                    # Get bracket names for team members (no emojis)
                    team_a_display = [get_player_display_name(player, interaction.guild.id) for player in team_a]
                    team_b_display = [get_player_display_name(player, interaction.guild.id) for player in team_b]
...
async def start(ctx):
    ...
    if tournament.mode == "2v2":
        ...
        teams_to_add = target_teams - current_teams
        if teams_to_add > 0:
            await ctx.send(f"Adding {teams_to_add} bot team(s) to complete the bracket...", delete_after=5)
            for i in range(teams_to_add):
                # Create bot team
                bot1_name = f"Bot{tournament.fake_count}"
                bot1_id = 761557952975420886 + tournament.fake_count
        ...
        players_to_add = target_players - len(tournament.players)
        if players_to_add > 0:
            await ctx.send(f"Adding {players_to_add} bot player(s) to complete the bracket...", delete_after=5)
            for i in range(players_to_add):
                bot_name = f"Bot{tournament.fake_count}"
                bot_id = 761557952975420886 + tournament.fake_count
                bot = FakePlayer(bot_name, bot_id)
    ...
    if tournament.mode == "2v2":
        for i, match in enumerate(current_round, 1):
            team_a, team_b = match
            # Get bracket names for team members (no emojis)
            team_a_display = [get_player_display_name(player, ctx.guild.id) for player in team_a]
            team_b_display = [get_player_display_name(player, ctx.guild.id) for player in team_b]
...
async def invite(ctx, member: discord.Member):
    ...
    view = TeamInvitationView(ctx.author, member)
...
async def commands(ctx):
    ...
    if has_permission(ctx.author, ctx.guild.id, 'tlr') or ctx.author.guild_permissions.manage_channels:
        embed.add_field(
    ...
    if has_permission(ctx.author, ctx.guild.id, 'htr') or has_permission(ctx.author, ctx.guild.id, 'tlr') or ctx.author.guild_permissions.manage_channels:
        embed.add_field(
